...
import { ExpenseCalculator } from "../zustand-store/ExpenseModel";
import { ZYearSection } from "./ZYearSection";
import { saveExpense, deleteExpense } from "../api/expenses";

export const ZExpenseDetailPanel = ({ 
  expenseId,
  onDeselect
}: { 
  ...
  const { getExpenseById, removeExpense, updateExpenseName } = useExpenseStore();
  ...
  const handleSave = async () => {
    ...
    try {
      const result = await saveExpense(ExpenseCalculator.toJSON(expense));
  ...
  return (
    <div className="bg-white rounded-xl shadow-md border border-gray-200">
      ...
      <div className="p-6 max-h-[calc(100vh-250px)] overflow-y-auto">
        <ZYearSection expense={expense} year={1} />
        <ZYearSection expense={expense} year={2} />
        <ZYearSection expense={expense} year={3} />
      </div>
    </div>
  );
};
